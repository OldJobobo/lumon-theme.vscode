...
- `themes/lumon-color-theme.json`: primary source of truth for UI and syntax colors.
- `README.md`: quick install/build notes for contributors.
- `PUBLISHING.md`: release and Marketplace workflow.
- `lumon-theme-*.vsix`: packaged artifact; do not edit manually.
...
- `unzip -l lumon-theme-*.vsix`: inspect package contents before publishing.
- `code --install-extension oldjobobo.lumon-theme`: install from Marketplace for verification.
- `code --install-extension ./lumon-theme-*.vsix`: install a local package for manual verification.
...
- File naming pattern: lowercase with hyphens (example: `lumon-color-theme.json`).
- Validate JSON before commit (example: `jq . themes/lumon-color-theme.json >/dev/null`).
